...
public class SpellChecker2 {
    ...
    public double calculateEM(String word, String correction, double editDistance) {
        ...
        else if (editDistance <= 1.0) {
            em = 1000;
        }
        else if (editDistance <= 2.0) {
            em = 500;
        }
        else {
            em = 50;
        ...
        // Silent Letters
        if(correction.contains("c")) {
            em *= 100;
        }
        if(correction.contains("h")) {
            em*= 100;
        }       
        if(correction.contains("w")) {
            em*= 100;
        }
        if(correction.contains("y")) {
            em*= 100;
        }

        // Similar letters
        for(int i = 0; i < word.length() && i < correction.length(); ++i) {
            if(word.charAt(i) == 'k' && correction.charAt(i) == 'q' ||
               word.charAt(i) == 'q' && correction.charAt(i) == 'k' ||
               word.charAt(i) == 's' && correction.charAt(i) == 'c' ||
               word.charAt(i) == 'c' && correction.charAt(i) == 's' ||
               word.charAt(i) == 'j' && correction.charAt(i) == 'g' ||
               word.charAt(i) == 'g' && correction.charAt(i) == 'j') {
                em *= 100;
            }
        }
    ...
    */
    private Map < String, Double > EditDist1(String word) {
        ...
        for (int i = 0; i <= word.length(); ++i) {
            ...
            if (L.length() > 0){
               for (int j = 0; j < alphabet.length(); ++j) {
                    temp = L.substring(0, i-1) + alphabet.charAt(j) + R;
                    cands.put(temp, 1.0); // 1.0 indicates the edit distance 1
                } 
            }
